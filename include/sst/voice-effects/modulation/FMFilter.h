...
template <typename VFXConfig> struct FMFilter : core::VoiceEffectTemplateBase<VFXConfig>
{
    ...
    basic_blocks::params::ParamMetaData paramAt(int idx) const
    {
        ...
        switch (idx)
        {
            case fpFreqL:
                return pmd()
                    .asAudibleFrequency()
                    .withName("Frequency L");
            case fpFreqR:
                return pmd()
                    .asAudibleFrequency()
                    .withName("Frequency R");
            case fpDepthL:
                return pmd()
                    .asFloat()
                    .withRange(0.f, 1.f)
                       ...
                       float pitch)
    {
        ...
        mSinOsc.setRate(440.0 * 2 * M_PI * this->note_to_pitch_ignoring_tuning(pitch + getRatio()) * this->getSampleRateInv());
        ...
        for (auto i = 0; i < VFXConfig::blockSize; ++i)
        {
            ...
            filter.setCoeff(mode, modFreqL, modFreqR, res, res, VFXConfig::getSampleRateInv(this), 0.f, 0.f);
