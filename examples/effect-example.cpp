...
template <typename FXT> int exampleHarness(const CLIArgBundle &arg)
{
    ...
    {
        float inputL alignas(16)[blockSize],
            inputR alignas(16)[blockSize],
            outputL alignas(16)[blockSize],
            outputR alignas(16)[blockSize];
        ...
        // sst::effects process buffers inplace
        for (size_t s = 0; s < blockSize; s++) {
            outputL[s] = inputL[s];
            outputR[s] = inputR[s];
        }
...
#define ADDTYPE(key, cls)                                                                          \
    {                                                                                              \
        ...
        if (fxType == std::string(key))                                                            \
            return exampleHarness<sst::effects::cls<ConcreteConfig>>(arg);                  \
    }
    ADDTYPE("floaty", floatydelay::FloatyDelay);
    ADDTYPE("bonsai", bonsai::Bonsai);
    ADDTYPE("reverb2", reverb2::Reverb2);
